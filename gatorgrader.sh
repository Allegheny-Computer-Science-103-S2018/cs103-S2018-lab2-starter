...
if [ "$CHECK" = true ]; then
  ...
  determine_exit_code $? "mdl src/www/html/answers.md"
  proselint README.md
  ...
  determine_exit_code $? "proselint src/www/html/answers.md"
  echo ""
  ...
  python3 gatorgrader/gatorgrader.py --nowelcome --directories src/www/html \
                                     --checkfiles answers.md --fragments "# A Web Site Created" --fragmentcounts 1
  determine_exit_code $?
  # --> GatorGrader CHECK: at least k subheader fragment exists in the code
  python3 gatorgrader/gatorgrader.py --nowelcome --directories src/www/html \
                                     --checkfiles answers.md --fragments "##" --fragmentcounts 5
  determine_exit_code $?
  # --> GatorGrader CHECK: at least k code block end fragment exists in the code
  python3 gatorgrader/gatorgrader.py --nowelcome --directories src/www/html \
                                     --checkfiles answers.md --fragments "\`\`\`" --fragmentcounts 8
  determine_exit_code $?
  ...
  determine_exit_code $?
  # --> GatorGrader CHECK: the answers file contains at least 6 paragraphs
